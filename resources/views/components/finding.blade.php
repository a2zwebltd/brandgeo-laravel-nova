...
@endphp
<div class="rounded-xl border-l-2 {{ $border }} bg-zinc-50 p-3 dark:bg-white/[0.03]">
    <div class="flex items-center gap-2">
        <span class="{{ $iconColor }} text-xs">{{ $glyph }}</span>
        <p class="text-sm font-bold">{{ $finding['title'] ?? '' }}</p>
        <span class="ml-auto rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-500 dark:bg-white/5">{{ $category }}</span>
    </div>
    <p class="mt-1 text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">{{ $finding['description'] ?? '' }}</p>
</div>
