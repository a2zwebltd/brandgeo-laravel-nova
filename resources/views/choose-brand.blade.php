<x-brandgeo-nova::layout title="Choose your brand — BrandGEO">
    <div class="mx-auto mt-8 max-w-2xl">
        <div class="mb-6 text-center">
            ...
            <p class="mt-1 text-sm text-zinc-400">Connected as {{ $account->email }} — pick which brand this dashboard should open with.</p>
        </div>

        <div class="space-y-3">
            @foreach ($brands as $brand)
                <div class="flex items-center gap-4 rounded-2xl border border-white/10 bg-zinc-900/70 p-5 transition hover:border-violet-500/40">
                    ...
                    <div class="flex shrink-0 flex-col gap-2">
                        ...
                        <a href="{{ route('brandgeo-nova.dashboard', array_filter(['brand' => $brand->uuid, 'embedded' => request('embedded')])) }}"
                           class="rounded-lg border border-white/10 px-4 py-1.5 text-center text-xs font-semibold text-zinc-300 hover:bg-white/5">
                            View once
                        </a>
                    </div>
                </div>
